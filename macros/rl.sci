...
global marked_handle g// handle to mark poles on rootlocus
...
if type(n)==2 & type(d)==2 & type(denom(n))==1 & type(denom(d))==1 & size(n)==[1 1] & size(d)==[1 1] then,  
			...
			return gtr	
				...
				d=pol2str(denom(gold));
				n=pol2str(numer(gold));
	end;
	end;
end;
